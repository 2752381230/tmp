...
段落1开始段落1开始段落1开始段落1开始段落1开始段落开始1段落开始1\n
...
+ 条目中包含引用
  ...
  > 引用的话必须4个 \s or1个 \t 开始,然后引用处理;
  > 非空行间隔和正常的引用一样处理;
  > 该行同样append到上一行尾部;
  > 该行尾部以\s{2,}\r作为结尾  
  > 该行单独展示
  
  > 中间以空行作为间隔，将作为单独一个引用展示
 + 代码部分,8个 \s or 2个 \t
    ```shell
    #!/bin/bash
...
- 视为软链 `shell:  ln -s`
- 锚点可在文件中的任何单独一行
- Github 上锚点失效：锚点的上一行即使 \s{2,}\r or 显示`<br />`作为结尾; 但是多个连续锚点没问题
